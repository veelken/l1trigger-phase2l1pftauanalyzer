#ifndef L1Trigger_TallinnL1PFTauAnalyzer_L1PFTauAnalyzerSignal_h
#define L1Trigger_TallinnL1PFTauAnalyzer_L1PFTauAnalyzerSignal_h
...
class L1PFTauAnalyzerSignal : public edm::EDAnalyzer 
{
 public:
  // constructor 
  explicit L1PFTauAnalyzerSignal(const edm::ParameterSet&);
    
  // destructor
  ~L1PFTauAnalyzerSignal();
 ...
 private:
  ...
  edm::EDGetTokenT<l1t::L1PFTauCollection> tokenNumerator_;
  ...
  struct efficiencyPlotEntryType
  {
    efficiencyPlotEntryType(double min_pt, double max_pt, double min_absEta, double max_absEta, const std::string& decayMode, 
			    double ptThreshold, const std::string& isolation)
      ...
      , dRmatch_(0.3)
    {
      ...
      else throw cms::Exception("L1PFTauAnalyzerSignal") 
	...
	for ( auto numeratorTau : numeratorTaus )
	{
  	  if ( (ptThreshold_       < 0. || numeratorTau.pt()            >=  ptThreshold_                         ) &&
	       ( isolation_ == kNone                                                                             || 
		(isolation_ == kVLooseIso && numeratorTau.passVLooseIso())                                       ||
		(isolation_ == kLooseIso  && numeratorTau.passLooseIso())                                        ||
		(isolation_ == kMediumIso && numeratorTau.passMediumIso())                                       ||
		(isolation_ == kTightIso  && numeratorTau.passTightIso())                                        ) )
	  {
